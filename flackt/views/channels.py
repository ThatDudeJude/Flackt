...
from flask import Blueprint, jsonify, g, redirect, request, url_for, render_template, session
...
def create_channel():
    ...
    if channel_name == '' or channel_topic =='' or channel_description == '' or channel_name.isspace() or channel_topic.isspace() or channel_description.isspace(): 
        error = True
    ...
    if not error:
        ...
        channel = add_new_channel(channel_name, channel_topic, channel_description, channel_creator)                
...
def get_channels(type):    
    ...
    if type == 'all':
        ...
        if type == 'non-member':
            non_member = users[g.user_name].get_non_member_channels(all_channels_info_names)        
...
def leaving_channel(channel_name):
    ...
    emit('Left Room', {'memberName': f"{session['current_user']}", "isLive": False}, to=channel_name, broadCast=True)
    leave_room(channel_name)
